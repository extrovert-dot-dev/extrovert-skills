...
Use the effective review policy returned by `get_inbox`. Existing human-recipient
and internal-email exceptions do not grant extra access and never skip writing
rules, intent, suppression or composition-token checks. The backend decides whether
an authorized exception applies. Do not change settings or split recipients to evade
review. Protected onboarding practice still requires review.
...
For sender display-name changes, use `extrovert-manage-inboxes`. Never invent a From
header, change the sender address or assume that a name proves identity.
