...
for (const pkg of inventory.packages ?? []) {
  ...
  if (pkg.profile === "full") check(same(pkg.skills, standalone.skills ?? []), `${pkg.name}: missing full skill`);
    ...
    check(same(pkg.skills, ["extrovert-connect", "extrovert-read-inbox", "extrovert-send-email", "extrovert-writing-rules", "wait-for-otp", "extrovert-support"]), "assistant six-skill inventory mismatch");
